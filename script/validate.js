        ...
        itemPaths.forEach((itemPath) => {
            ...
            dataArray.forEach((itemData) => {
                Joi.validate(itemData, itemSchema, (error) => {
                    if (error) {
                        errorList.push(new Error(`[ITEM] ${path.relative(ITEMS_DIRECTORY, itemPath)} ${itemData.title}: ${error.message}`));
                    }
                });
                ...
                if (itemData.end && itemData.begin) {
                    ...
                    if (endDate < beginDate) {
                        errorList.push(new Error(`[ITEM] ${path.relative(ITEMS_DIRECTORY, itemPath)} ${itemData.title}: End is earlier than begin`));
                    }
                }
                ...
                if (itemData.sites) {
                    ...
                    if ([...new Set(siteNames)].length !== siteNames.length) {
                        errorList.push(new Error(`[ITEM] ${path.relative(ITEMS_DIRECTORY, itemPath)} ${itemData.title}: Sites is not unique`));
                    }
                }
            });
        });
        ...
        itemPaths.forEach((itemPath) => {
            ...
            dataArray.forEach((itemData) => {
                ...
                if (idMap[key]) {
                    ...
                    errorList.push(new Error(`[DUPLICATE] Bangumi ID ${id} is duplicated in ${paths.join(' and ')}`));
                }
                idMap[key] = itemPath;
            });
        });
...
Promise.all([
    ...
    .then((...errorLists) => {
        ...
        if (errorList.length) {
            ...
            errorList.forEach((error) => { console.error(error.message); });
            console.log('');
            process.exit(1);
        } else {
